...
public class MidnightConfig {
    ...
    private static void initClient(String modid, Field field, EntryInfo info) {
        ...
        if (e != null)
            if (type == int.class) textField(info, Integer::parseInt, INTEGER_ONLY, e.min(), e.max(), true);
                ...
                Function<Object, Text> func = value -> Text.translatable(info.value.toString());
                info.widget = new AbstractMap.SimpleEntry<ButtonWidget.PressAction, Function<Object, Text>>(button -> {
    ...
    private static class MidnightConfigScreen extends Screen {
        ...
        protected MidnightConfigScreen(Screen parent, String modid) {
            super(Text.translatable("SkinSwapper"));
            this.parent = parent;
            this.modid = modid;
        ...
        protected void init() {
            ...
            for (EntryInfo info : entries) {
                if (info.id.equals(modid)) {
                    Text name = Text.translatable(translationPrefix + info.field.getName());
                    ButtonWidget resetButton = new ButtonWidget(width - 155, 0, 40, 20, Text.literal("Reset").formatted(Formatting.RED), (button -> {
